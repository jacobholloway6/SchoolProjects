...
public:
  CityNode(string city) { //Takes in the new city 
    this->city = city;
  }
  ...
  inline vector<string> getDirectRoutedCities() { // Grabs the routed cities which is a vector so there should be a value. 
  ...
  inline void setReachableCities(vector<string> rCities) { //everything uses strings except for the this which takes in a vector of cities and then sets it to the reachable cities
                                                          // This is used once we know what city we are dealing with an it has travered all options. 
